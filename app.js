...
function setupNavigation() {
    ...
    if (currentUser.role === 'admin') {
        ...
        // Hide admin buttons
        const adminBtns = ['addPropertyBtn', 'addTenantBtn', 'addPaymentBtn'];
        adminBtns.forEach(id => {
            const btn = document.getElementById(id);
...
const Properties = {
                    ...
                    ${properties.map(p => {
                        ...
                        return `
                            <tr>
                                ...
                                <td>
                                    <button class="btn-danger-sm" onclick="Properties.delete('${p.id}')">Delete</button>
                                </td>
                            </tr>
                        `;
                    }).join('')}
    ...
    delete: (id) => {
        if (confirm('Delete this property?')) {
            ...
            Properties.render();
            Dashboard.load();
            Utils.showAlert('Property deleted!');
        }
    }
};
...
const Tenants = {
                    ...
                    ${tenants.map(t => {
                        ...
                        return `
                            <tr>
                                ...
                                <td>
                                    <button class="btn-danger-sm" onclick="Tenants.delete('${t.id}')">Delete</button>
                                </td>
                            </tr>
                        `;
                    }).join('')}
    ...
    delete: (id) => {
        if (confirm('Delete this tenant?')) {
            ...
            Tenants.render();
            Dashboard.load();
            Utils.showAlert('Tenant deleted!');
        }
    }
};
...
const RenterView = {
    load: () => {
        ...
        // My Property
        document.getElementById('myPropertyDetails').innerHTML = property ? `
            <div class="alert alert-info">
                <div>
                    <strong>${property.name}</strong><br>
                    ${property.address}<br>
                    Rent: ${Utils.formatCurrency(property.rent)}/month<br>
                    Lease: ${Utils.formatDate(tenant.leaseStart)} - ${Utils.formatDate(tenant.leaseEnd)}
                </div>
            </div>
